...
def add_address(payload: ContactPayload) -> Contact:
    return set_address(payload)


@response(InfoMessages.ADDRESS_DELETED)
@write_data(entity=EntitiesType.CONTACTS)
def delete_address(payload: ContactPayload) -> Optional[Contact]:
...
@write_data(entity=EntitiesType.CONTACTS)
def update_address(payload: ContactPayload) -> Contact:
    return set_address(payload)
