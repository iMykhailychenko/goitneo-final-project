...
def test_search_by_name(setup_db):
    ...
    result = controller(Actions.SEARCH, SearchPayload(entity=EntitiesType.CONTACTS, query="jo"))
...
def test_search_by_phone(setup_db):
    ...
    result = controller(Actions.SEARCH, SearchPayload(entity=EntitiesType.CONTACTS, query="1111"))
    assert len(result.value) == 1
...
def test_search_by_birthday(setup_db):
    ...
    result = controller(Actions.SEARCH, SearchPayload(entity=EntitiesType.CONTACTS, query="1995"))
    assert len(result.value) == 1
...
def test_search_by_email(setup_db):
    ...
    result = controller(Actions.SEARCH, SearchPayload(entity=EntitiesType.CONTACTS, query="test@"))
    assert len(result.value) == 1
