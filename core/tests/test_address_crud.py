...
from core.models import ContactPayload, EntitiesType
...
def test_add_address(setup_test_user):
    ...
    assert db.select(entity=EntitiesType.CONTACTS, key="Joe").address == address_value
...
def test_delete_address(setup_test_user):
    ...
    assert len(db.select(entity=EntitiesType.CONTACTS, key="Joe").address) == 0
...
def test_update_address(setup_test_user):
    ...
    assert (
        db.select(entity=EntitiesType.CONTACTS, key="Joe").address == new_address_value
    )
